...
/// Lists paid/refunded orders as a simple transaction ledger. A full build
/// would pull richer transaction data (fees, card brand) straight from
/// Stripe via an Edge Function; this reads what's already mirrored into
/// `orders` by the `stripe-webhook` function.
      ...
      body: ordersAsync.when(
        data: (orders) {
          final transactions = orders
              .where((o) => o.stripePaymentIntentId != null && o.stripePaymentIntentId!.isNotEmpty)
              .toList();
          ...
          final totalRevenue = transactions
              .where((o) => o.status != OrderStatus.refunded && o.status != OrderStatus.cancelled)
                ...
                child: Card(
                  ...
                  child: Padding(
                    ...
                    child: Row(
                      ...
                      children: [
                        const Text('Total Revenue', style: TextStyle(color: AppColors.white)),
