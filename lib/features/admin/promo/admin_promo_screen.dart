...
class AdminPromoScreen extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return Scaffold(
      ...
      body: promosAsync.when(
                  ...
                  subtitle: Text(
                    [
                      ...
                      p.maxRedemptions != null
                          ? 'Used ${p.timesRedeemed}/${p.maxRedemptions}'
                          : 'Used ${p.timesRedeemed}× · unlimited',
                      if (p.expiresAt != null)
                        'Expires ${DateFormat.yMMMd().format(p.expiresAt!)}',
                      if (!p.active) 'Inactive',
                      if (expired) 'Expired',
                      if (p.isRedemptionCapReached) 'Limit reached',
                    ].join(' · '),
                  ),
                  trailing: Row(
                      ...
                      IconButton(
                        ...
                        onPressed: () async {
                          final confirmed = await confirmDestructiveAction(
                            ...
                            message: 'This permanently removes "${p.code}". This cannot be undone.',
                          );
                          if (!confirmed) return;
        ...
        error: (e, _) => ErrorState(onRetry: () => ref.invalidate(adminPromoCodesProvider)),
      ),
    );
  }
  ...
  void _showForm(BuildContext context, WidgetRef ref, {PromoCode? existing}) {
    ...
    final valueCtrl = TextEditingController(
      text: existing?.discountValue.toStringAsFixed(2),
    );
    ...
    // Created outside the sheet's own widget tree (this is a plain method,
    // not a State), so nothing disposes them automatically — do it
    // ourselves once the sheet closes, whichever way it closes.
            ...
            child: Column(
              ...
              children: [
                ...
                DropdownButtonFormField<DiscountType>(
                  ...
                  onChanged: (v) =>
                      setSheetState(() => type = v ?? DiscountType.percent),
                ...
                TextField(
                  ...
                  decoration: const InputDecoration(
                    ...
                    hintText: 'e.g. 100 — leave blank for unlimited',
                  ),
                ),
                if (existing != null) ...[
                  const SizedBox(height: 4),
                  Text(
                    'Used ${existing.timesRedeemed} time(s) so far.',
                    style: const TextStyle(fontSize: 12, color: AppColors.grey),
                  ),
                ],
                const SizedBox(height: 4),
                const Text(
                  'Every code is one-use-per-customer automatically — this '
                  'limit only caps the total number of customers who can use it.',
                  ...
                  onPressed: () async {
                    ...
                    if (code.isEmpty || value <= 0) {
                      ScaffoldMessenger.of(ctx).showSnackBar(
                        const SnackBar(
                          content: Text('Enter a code and a discount value greater than 0.'),
                        ),
                      );
                      return;
                    }
                    // Not exploitable (the checkout total is already
                    // clamped to the subtotal both client- and
                    // server-side), but a percent code saved above 100
                    // reads as an admin data-entry mistake — catch it here.
                    if (type == DiscountType.percent && value > 100) {
                      ScaffoldMessenger.of(ctx).showSnackBar(
                        const SnackBar(
                          content: Text('Percent discount can\'t be more than 100%.'),
                        ),
                      );
                      return;
                    }
                    final maxRedemptionsText = maxRedemptionsCtrl.text.trim();
                    await ref.read(promoRepositoryProvider).upsert({
                      ...
                      'max_redemptions': maxRedemptionsText.isEmpty
                          ? null
